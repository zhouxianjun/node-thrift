        ...
        return new Promise(ok => {
            watch.on('file', async (root, fileStat, next) => {
                try {
                    ...
                    await this.loadObject(service, service.name || result.name.substring(0, result.name.length - '.js'.length));
                } catch (err) {
            ...
            watch.on('end', () => {this.emit('ready');ok();});
        });
    }

    async loadObject(service, name) {
        if (service && service.thrift && service.version) {
            ...
            await this.register(name, service.version, QS.stringify(address, null, null, {
                encodeURIComponent: (str) => {
                    return str;
                }
            }));
            this.processor.registerProcessor(name, new service.thrift.Processor(Reflect.construct(service, [])));
            logger.info(`register processor ${name}`);
        }
    }
};
