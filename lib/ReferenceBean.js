...
module.exports = class ReferenceBean {
    ...
    static async proxyMethod(target, that, args) {
        ...
        // 执行过滤器选择生产者
        ReferenceBean.filterMethod(that.filters, 'selector', false, ctx, routerResult, that.routerCache, that.configuratorCache, args);
        // 负载均衡选举
        ...
        let before = ReferenceBean.filterMethod(that.filters, 'before', true, ctx, invoker, args);
        if (before !== true) {
